...
export default defineConfig({
  ...
  server: {
    ...
    strictPort: true,
    proxy: {
      '^/(?:[^/]+/)?(logout|getTwoFactorEnable|csrf-token|server)(?:/|$)': makeBackendProxy(BACKEND_TARGET),
      '^/(?:[^/]+/)?panel/(api|setting|xray|csrf-token)(?:/|$)': makeBackendProxy(BACKEND_TARGET),
      '^/(?:[^/]+/)?client/api(?:/|$)': makeBackendProxy(BACKEND_TARGET),
      '^/(?:[^/]+/)?ws$': {
        target: BACKEND_WS,
        ws: true,
        changeOrigin: true,
